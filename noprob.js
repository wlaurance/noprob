...
(function() {
  var App, colors, exec, fs, program, watch, _,
  ...
  App = (function() {
    ...
    App.prototype.extras = function() {
      this.extensions = _.str.words(program.extension, '|');
      this.pollInterval = 500;
      this.lastTime = this.currentTime();
      this.gRelease = this.currentTime() + 1;
      return this.errRelease = this.currentTime();
    };

    App.prototype.commandLine = function() {
      return program.option('-x, --exec [command]', 'string to execute globally on any file change', '').option('-l, --local [command]', "string to execute locally on any file that's changed", '').option('-w, --watch [directory]', 'directory to watch', '.').option('-e, --extension [extensions]', 'list of file extensions to watch', '').option('-d, --dot', 'watch hidden dot files').parse(process.argv);
    };
